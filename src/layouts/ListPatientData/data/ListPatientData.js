...
function ListPatientData() {
  ...
  return (
    <SoftBox width="100%" maxWidth="1200px" mx="auto" p={3}>
      ...
      <SoftBox display="flex" flexDirection="column" gap={2}>
        {loading ? (
          <SoftTypography variant="h6" align="center" color="text.secondary">
            Loading data...
          </SoftTypography>
        ) : filteredPatients.length > 0 ? (
          filteredPatients.map((patient, index) => (
            <PatientCard key={patient.CIN} {...patient} index={index} />
          ))
        ) : (
          <SoftTypography variant="body1" align="center" color="text.secondary">
            No patients found.
          </SoftTypography>
        )}
      </SoftBox>
